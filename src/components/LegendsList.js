const legends = [
  {
    id: 1,
    title: "Poli'ahu and Pele's Sled Race",
    description: 
    "This epic race between the snow goddess Poliʻahu and volcano goddess Pele reveals how Hawaiʻi Island came to hold both icy peaks and fiery lands.",
    lat: 19.807189,
    lon: -155.092848,
  },
  {
    id: 2,
    title: "Maui and his Magical Hook  ",
    description: "Maui attempts to unite the islands together with his magical hook.",
    lat: 19.729485,
    lon: -155.068584,
  },
  {
    id: 3,
    title: "Mo'o Guardians of Hilo",
    description: "Lizard spirits who guard sacred pools.",
    lat: 19.719929627263326, 
    lon: -155.10944345767197,
  },
  {
    id: 4,
    title: "The Woman in the Moon",
    description: "The story of Hina, the moon goddess.",
    lat: 19.731490, 
    lon: -155.101102,
  },
  {
    id: 5,
    title: "Maui Traps the Sun",
    description: "Maui's legendary feat of slowing the sun to extend daylight.",
    lat: 19.7192,
    lon: -155.09,
  },
  {
    id: 6,
    title: "Sharkman of Waipi'o",
    description: "The tale of a sharkman who protects the valley and its people.",
    lat: 20.119622, 
    lon: -155.590681,
  },
  {
    id: 7,
    title: "Hi'ilawe and Hakalaoa",
    description: "Love story of Hi'ilawe and Hakalaoa.",
    lat: 20.098171735609657, 
    lon: -155.59917318082273,
  },
  {
    id: 8,
    title: "The Naupaka flower's Tale",
    description: "Love story of the Naupaka flower, symbolizing unrequited love.",
    lat: 20.119508, 
    lon: -155.592001,
  },
  {
    id: 9,
    title: "The story of the Naha Stone",
    description: "The story of how Kamehameha the Great used the Naha Stone to prove his strength and destiny.",
    lat: 19.724905418213705, 
    lon: -155.09061267844766,
  },
  {
    id: 10,
    title: "Maui and the 'Alae Birds",
    description: "The story of how Maui learned to make fire with the help of the 'Alae birds.",
    lat: 19.724885219592643, 
    lon: -155.09061267844766,
  },


];

export default legends;